...
public class ConsumerConfiguration {
    ...
    IntegrationFlow consumer() {
        ...
        log.info("starting consumer..  Access Point -> " + "topic : " + kafkaConfig.getTopic() + "zookpeeperaddress : "
                + kafkaConfig.getZookeeperAddress());
        ...
        KafkaHighLevelConsumerMessageSourceSpec messageSourceSpec = Kafka
                ...
                .consumerProperties(
                        props -> props.put("auto.offset.reset", "smallest").put("auto.commit.interval.ms", "1"))
                .addConsumer("iot-events-group",
        ...
        return IntegrationFlows.from(messageSourceSpec, endpointConfigurer)
                .<Map<String, List<String>>> handle((payload, headers) -> {
                    payload.entrySet().forEach(e -> log.info(readProducer(payload)));
                    return null;
                }).get();
    ...
    public String readProducer(Map<String, List<String>> payload) {
        try {
            ...
            for (int i = 0; i < value.get(0).size(); i++) {
                ...
                log.info(" producer data : " + value.get(0).get(i));

            }
        } catch (Exception e) {

            log.info("Ended in failure method ConsumerConfiguration@createEvents " + e.getMessage());
        }
        return "success";
    }
